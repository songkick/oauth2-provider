...
describe OAuth2::Provider::AccessToken do
  ...
  shared_examples_for "valid token" do
    ...
    it "does not add headers" do
      token.response_headers.should be == {}
    end
    it "has an OK status code" do
      token.response_status.should be == 200
    end
    it "returns the owner who granted the authorization" do
      token.owner.should be == @bob
    end
  end
  ...
  describe "when the authorization has expired" do
    ...
    it "returns an error response" do
      token.response_headers['WWW-Authenticate'].should be == "OAuth realm='Demo App', error='expired_token'"
      token.response_status.should be == 401
    end
  end
  ...
  describe "with a non-existent token" do
    ...
    it "returns an error response" do
      token.response_headers['WWW-Authenticate'].should be == "OAuth realm='Demo App', error='invalid_token'"
      token.response_status.should be == 401
    end
  end
  ...
  describe "with a token for the wrong user" do
    ...
    it "returns an error response" do
      token.response_headers['WWW-Authenticate'].should be == "OAuth realm='Demo App', error='insufficient_scope'"
      token.response_status.should be == 403
    end
  end
  ...
  describe "with no token string" do
    ...
    it "returns an error response" do
      token.response_headers['WWW-Authenticate'].should be == "OAuth realm='Demo App'"
      token.response_status.should be == 401
    end
  end
end
