...
describe OAuth2::Provider do
  ...
  describe "access grant request" do
    ...
    describe "when there is already a pending authorization from the user" do
      before do
        @authorization = OAuth2::Model::Authorization.create do |auth|
          auth.owner  = @owner
          auth.client = @client
          auth.code   = 'pending_code'
      ...
      it "immediately redirects with the code" do
        response = get(params)
        response.code.to_i.should be == 302
        response['location'].should be == 'https://client.example.com/cb?code=pending_code'
      end
      ...
      describe "when the client is requesting scopes it already has access to" do
        ...
        it "immediately redirects with the code" do
          response = get(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=pending_code&scope=offline_access'
        end
      end
      ...
      describe "and the authorization does not have a code" do
        ...
        it "generates a new code and redirects" do
          ...
          response = get(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=new_code'
        end
      end
    ...
    describe "when there is already a completed authorization from the user" do
      before do
        @authorization = OAuth2::Model::Authorization.create do |auth|
          ...
          auth.client = @client
          auth.code   = nil
          auth.access_token = OAuth2.hashify('complete_token')
        end
      end
      ...
      describe "for token requests" do
        ...
        it "immediately redirects with a new token" do
          ...
          response = get(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb#access_token=new_access_token'
        end
      ...
      it "does not create a new Authorization" do
        get(params)
        OAuth2::Model::Authorization.count.should be == 1
      end
    ...
    describe "with a client_id and a bad redirect_uri" do
      ...
      it "redirects to the client's registered redirect_uri" do
        response = get(params)
        response.code.to_i.should be == 302
        response['location'].should be == 'https://client.example.com/cb?error=invalid_request&error_description=Missing+required+parameter+response_type'
      end
    end
    ...
    describe "with an invalid request" do
      ...
      describe "with a state parameter" do
        ...
        it "redirects to the client, including the state param" do
          response = get(params)
          response.code.to_i.should be == 302
          response['location'].should be == "https://client.example.com/cb?error=invalid_request&error_description=Missing+required+parameter+response_type&state=Facebook%0Amesses+this%0Aup"
        end
      end
    end
  end
  ...
  describe "authorization confirmation from the user" do
    ...
    describe "without the user's permission" do
      ...
      it "redirects to the client with an error" do
        ...
        response.code.to_i.should be == 302
        response['location'].should be == 'https://client.example.com/cb?error=access_denied&error_description=The+user+denied+you+access'
      end
    end
    ...
    describe "with valid parameters and user permission" do
      ...
      describe "for code requests" do
        ...
        it "redirects to the client with an authorization code" do
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=foo'
        end
        ...
        it "passes the state parameter through" do
          ...
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=foo&state=illinois'
        end
        ...
        it "passes the scope parameter through" do
          ...
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=foo&scope=foo+bar'
        end
      end
      ...
      describe "for token requests" do
        ...
        it "redirects to the client with an access token" do
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb#access_token=foo&expires_in=10800'
        end
        ...
        it "passes the state parameter through" do
          ...
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb#access_token=foo&expires_in=10800&state=illinois'
        end
        ...
        it "passes the scope parameter through" do
          ...
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb#access_token=foo&expires_in=10800&scope=foo+bar'
        end
      end
      ...
      describe "for code_and_token requests" do
        ...
        it "redirects to the client with an access token" do
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=foo#access_token=foo&expires_in=10800'
        end
        ...
        it "passes the state parameter through" do
          ...
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=foo&state=illinois#access_token=foo&expires_in=10800'
        end
        ...
        it "passes the scope parameter through" do
          ...
          response = allow_or_deny(params)
          response.code.to_i.should be == 302
          response['location'].should be == 'https://client.example.com/cb?code=foo#access_token=foo&expires_in=10800&scope=foo+bar'
        end
      end
    end
  end
    ...
    describe "using authorization_code request" do
      ...
      describe "when there is an Authorization with code and token" do
        ...
        it "exchanges the code for the new token on the existing Authorization" do
          ...
          @authorization.access_token_hash.should be == OAuth2.hashify('random_access_token')
        end
      end
    end
  end
  ...
  describe "protected resource request" do
    ...
    shared_examples_for "protected resource" do
      ...
      it "allows access when the key is passed" do
        ...
        JSON.parse(response.body)['data'].should be == 'Top secret'
        response.code.to_i.should be == 200
      end

      it "blocks access when the wrong key is passed" do
        response = request('/user_profile', 'oauth_token' => 'is-the-password-books')
        JSON.parse(response.body)['data'].should be == 'No soup for you'
        response.code.to_i.should be == 401
        response['WWW-Authenticate'].should be == "OAuth realm='Demo App', error='invalid_token'"
      end
      ...
      it "blocks access when the no key is passed" do
        ...
        JSON.parse(response.body)['data'].should be == 'No soup for you'
        response.code.to_i.should be == 401
        response['WWW-Authenticate'].should be == "OAuth realm='Demo App'"
      end
      ...
      describe "enforcing SSL" do
        ...
        it "blocks access when not using HTTPS" do
          ...
          JSON.parse(response.body)['data'].should be == 'No soup for you'
          response.code.to_i.should be == 401
          response['WWW-Authenticate'].should be == "OAuth realm='Demo App', error='invalid_request'"
        end

        it "destroys the access token since it's been leaked" do
          authorization.access_token_hash.should be == OAuth2.hashify('magic-key')
          request('/user_profile', 'oauth_token' => 'magic-key')
          authorization.reload
