...
<tt>oauth2/provider/model/schema.rb</tt>. Run it to update your database to
store OAuth data:
...
since they are the client's access credentials.
...
the OAuth service through the path <tt>/oauth/authorize</tt>.
